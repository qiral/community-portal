...
export async function addUsertoClub(clubID: string, userID: string, role: string) {
  try {
    const response = await MakeAuthenticatedRequest('/club-user', 'POST', {
      club_id: clubID,
      user_id: userID,
      role: role,
    })
...
export async function updateUserClubRole(clubID: string, userID: string, role: string) {
  try {
    const response = await MakeAuthenticatedRequest('/club-user', 'PUT', {
      club_id: clubID,
      user_id: userID,
      role: role,
    })
...
export async function removeUserfromClub(clubID: string, userID: string) {
  try {
    const response = await MakeAuthenticatedRequest('/club-user', 'DELETE', {
      club_id: clubID,
      user_id: userID,
    })
    if (response instanceof Error) {
